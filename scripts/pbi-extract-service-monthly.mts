 ...
 * Uses the correct join format with dServiços entity.
 *
 * Usage: npx tsx scripts/pbi-extract-service-monthly.mts
 */
...
const OUTPUT = "/tmp/pbi_service_monthly";
...
async function main() {
  ...
  for (let mes = 0; mes < 6; mes++) {
    ...
    for (const servico of SERVICOS) {
      // Use _MedidasFernando.Faturamento with dServiços join
      const r = await query(
      ...
      results[mesName][servico] = val !== null ? Number(val) : null;
      console.log(`  ${mesName} / ${servico}: ${val}`);
    }
  }

  // Also query ticket médio por serviço por mês
  console.log("\n=== Querying ticket médio por serviço by month ===");
  const resultsTM: Record<string, Record<string, number | null>> = {};
  for (let mes = 0; mes < 6; mes++) {
    const mesName = MES_NAMES[mes];
    resultsTM[mesName] = {};
    ...
    for (const servico of SERVICOS) {
      const r = await query(
        [{ Measure: { Expression: { SourceRef: { Source: "f" } }, Property: "Ticket Médio Serviço Selecionado" }, Name: "_MedidasFernando.Ticket Médio Serviço Selecionado" }],
        [
          { Name: "f", Entity: "_MedidasFernando", Type: 0 },
          { Name: "s", Entity: "dServiços", Type: 0 },
          { Name: "l", Entity: "LocalDateTable_44a31fe3-dfff-4cb2-8b37-9237320f84e9", Type: 0 },
      ...
      const val = extractScalar(r);
      resultsTM[mesName][servico] = val !== null ? Number(val) : null;
      console.log(`  ${mesName} / ${servico} TM: ${val}`);
    }
  }

  writeFileSync(join(OUTPUT, "service_monthly.json"), JSON.stringify({ faturamento: results, ticketMedio: resultsTM }, null, 2));
