/**
 * Extract all main KPIs by month for 2026.
 * Uses the same API replay technique as pbi-fix-filters.mts.
 *
 * Usage: npx tsx scripts/pbi-extract-monthly.mts
 */
...
async function main() {
  ...
  for (let mes = 0; mes < 6; mes++) {
    ...
    for (const m of monthMeasures) {
      ...
      results[mesName][m.name] = val;
      console.log(`  ${mesName} / ${m.name}: ${val}`);
    }
  }

  // Query faturamento por canal por mês
  console.log("\n=== Querying faturamento por canal by month ===");
  const canais = ["Retenção", "Ativo", "Orgânico", "Indicação", "Crossell", "Pago"];
  for (let mes = 0; mes < 6; mes++) {
    const mesName = MES_NAMES[mes];
    if (!results[mesName]) results[mesName] = {};
    results[mesName]["faturamentoPorCanal"] = {};
    ...
    for (const canal of canais) {
      ...
      const val = extractScalar(r);
      (results[mesName]["faturamentoPorCanal"] as Record<string, unknown>)[canal] = val;
      console.log(`  ${mesName} / ${canal}: ${val}`);
    }
  }

  // Query faturamento por serviço por mês
  console.log("\n=== Querying faturamento por serviço by month ===");
  const servicos = ["Nutrologia", "Pediatria", "Dermatologia"];
  for (let mes = 0; mes < 6; mes++) {
    const mesName = MES_NAMES[mes];
    if (!results[mesName]) results[mesName] = {};
    results[mesName]["faturamentoPorServico"] = {};
    ...
    for (const servico of servicos) {
      ...
      const val = extractScalar(r);
      (results[mesName]["faturamentoPorServico"] as Record<string, unknown>)[servico] = val;
      console.log(`  ${mesName} / ${servico}: ${val}`);
    }
  }

  writeFileSync(join(OUTPUT, "monthly_data.json"), JSON.stringify(results, null, 2));
  console.log(`\nSaved to ${join(OUTPUT, "monthly_data.json")}`);
}
